package com.test.sort;

/**
 * @Auth 45208
 * @Date 9/15/2020
 * 快速排序
 * 快排也是使用分而治之的思想
 * 首先我们在要排序的数组中从开始下标到结束下标的任意位置找一个元素作为分界元素，将比这个元素小的放到数组的左边，将比这个元素大的放到这个数组的右边，最后将这个分界放到数组的中间位置
 * 然后我们对于分界左边的数组进行同样操作进行排序，对分界右边的数组也进行排序，然后重复上面的动作，直到要排序的数组只有一个元素的时候，我们认为这个数组是有序的，那么对于整体而言也是有序的了
 * 最关键的地方是我们如何选择分界元素和如何对数组分区，也就是将小于分界放到数组左边，大于数组分区放到数组右边
 * 首先我们可以选择要排序数组的最后一个元素作为分界
 * 然后我们可以使用像之前插入排序的思想，首先使用最后一个元素作为轴，然后对数据进行分区，分区方法是分别用两个指针，第一个指针用来遍历整个要排序的区间，来查找小于或者大于轴的数据，第二个指针用来维护小于或者大于轴的的数据的区间，指向区间的后一个位置
 * 当第一个指针查找到大于或者小于，也就是满足条件的数据，就把当前这个数据和第二个指针维护的区间的下一个位置进行交换，然后将第二个指针向前移动一位，移动到当前符合要求区间的下一位，直到第一个指针移动到倒数第二位数据结束，然后将第二个指针指向的数据和轴
 * 进行交换，这样分区就结束了，最后的效果就是轴左边的都是小于或者大于轴的数据，轴的右边都是大于或者小于轴的数据。
 **/
public class QuickSort {

    public void sort(int[] arr, int n) {
        if (n <= 1) {
            return;
        }
        quickSort(arr, 0, n - 1);
    }

    public void quickSort(int[] arr, int s, int e) {
        if (s >= e) {
            return;
        }
        int q = partition(arr, s, e);
        quickSort(arr, s, q - 1);
        quickSort(arr, q + 1, e);
    }

    public int partition(int[] arr, int s, int e) {
        int i = s;
        int j = s;
        int pivot = arr[e];
        for (; j <= e - 1; j++) {
            if (arr[j] < pivot) {
                int tmp = arr[j];
                arr[j] = arr[i];
                arr[i] = tmp;
                i++;
            }
        }
        arr[e] = arr[i];
        arr[i] = pivot;
        return i;
    }
}
